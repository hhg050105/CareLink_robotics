...
std::vector<hardware_interface::StateInterface> DiffDriveArduinoHardware::export_state_interfaces()
{
  ...
  state_interfaces.emplace_back(hardware_interface::StateInterface(
    "carelink_status", "battery_voltage", &battery_voltage_));
  state_interfaces.emplace_back(hardware_interface::StateInterface(
    "carelink_status", "dock_state", &dock_state_));
...
/* Battery and dock state do not need to be polled at control-loop speed.
 * Read them approximately once per second.
 */
  status_elapsed_sec_ += period.seconds();

  if (status_elapsed_sec_ >= 1.0)
  {
    battery_voltage_ = comms_.read_battery_voltage();
    dock_state_ = comms_.read_dock_state() ? 1.0 : 0.0;

    status_elapsed_sec_ = 0.0;
  }
